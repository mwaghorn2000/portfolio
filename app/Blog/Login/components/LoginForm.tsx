...
export default function LoginForm() {
    ...
    return <main className="blog-wrap max-w-5xl">
        ...
        <div className="mt-10 grid overflow-hidden rounded-3xl border border-stone-200 bg-white md:grid-cols-2">
            <div className="flex flex-col justify-between bg-stone-900 p-8 text-white sm:p-12"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-lime-300">Blog administration</p><h1 className="mt-8 text-4xl font-extrabold leading-tight tracking-tight">Manage and<br /><span className="text-lime-300">publish posts.</span></h1></div><p className="mt-8 max-w-xs text-sm leading-7 text-stone-300">Sign in to create, edit, preview, and delete blog posts.</p></div>
            <form onSubmit={submit} className="space-y-6 p-8 sm:p-12"><div><h2 className="text-2xl font-bold">Welcome back</h2><p className="blog-muted mt-2">Sign in to your author dashboard.</p></div>
                ...
                <button className="blog-button w-full" disabled={busy}>{busy ? 'Signing in…' : 'Open dashboard →'}</button><p className="blog-muted text-center">Author access only.</p>
            </form>
        </div>
    </main>;
}
