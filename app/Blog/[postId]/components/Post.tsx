...
export default function Post({ _id }: { _id: string }) {
    ...
    useEffect(() => {
        ...
        fetch(`/api/posts/${_id}`, { signal: controller.signal })
            .then(async response => { if (!response.ok) throw new Error(response.status === 404 ? 'This entry could not be found.' : 'This entry could not be loaded. Please try again.'); return response.json(); })
    ...
    return <main className="blog-wrap max-w-4xl">
        ...
        {error ? <div role="alert" className="blog-error mt-8">{error}</div> : !post ? <p role="status" className="blog-muted mt-8">Loading entry…</p> : <article>
            ...
            <div className="mt-12 border-t border-stone-200 pt-7"><Link href="/Blog" className="blog-secondary">Explore more entries →</Link></div>
        </article>}
    </main>;
}
