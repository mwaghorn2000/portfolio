...
export default function Dashboard() {
    ...
    return <main className="blog-wrap">
        <header className="flex flex-wrap items-end justify-between gap-6">
            <div><p className="blog-eyebrow">Your space to write</p><h1 className="blog-title">Writing studio<span className="text-lime-600">.</span></h1><p className="mt-4 text-stone-600">Turn what you&apos;re learning into your next entry.</p></div>
            <Link href="/Blog/Dashboard/CreatePost" className="blog-button">+ New entry</Link>
        </header>
        <div className="my-9 grid gap-3 sm:grid-cols-3">
            <div className="blog-panel"><p className="blog-muted">Published entries</p><p className="mt-2 text-3xl font-bold">{loading || error ? '—' : posts.length}</p></div>
            ...
            <div className="rounded-2xl bg-stone-900 p-6 text-white"><p className="text-sm font-semibold text-lime-300">A little progress, written down.</p><p className="mt-2 text-sm leading-6 text-stone-300">Start an entry, preview your Markdown, and publish when you&apos;re ready.</p></div>
        </div>
        <section aria-labelledby="manage-heading">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-4"><h2 id="manage-heading" className="text-xl font-bold">Manage entries</h2><div className="flex w-full flex-wrap gap-3 sm:w-auto">
